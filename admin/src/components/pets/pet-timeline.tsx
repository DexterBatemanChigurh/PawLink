...
export function PetTimeline({ petId }: PetTimelineProps) {
  ...
  if (isLoading) {
    return <div className="text-gray-500 text-sm py-4">Carregando histórico...</div>
  }
  ...
  return (
    <div className="space-y-0">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">📋 Histórico do Pet</h3>
          ...
          {events.map((event) => {
            ...
            return (
              <div key={event.id} className="relative pl-10">
                <div
                  className="absolute left-[11px] w-[18px] h-[18px] rounded-full border-2 border-white shadow-sm flex items-center justify-center text-[10px]"
                ...
                <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                  ...
                  {event.vetName && (
                    <p className="text-xs text-gray-500 mt-1">👨‍⚕️ {event.vetName}</p>
                  )}
                </div>
              </div>
            )
